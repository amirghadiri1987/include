...
private:
   ...
   static bool       s_list_shown;        // Static flag to track if any list is shown
...
public:
   ...
   bool              CheckShowHide(void) {return flage;}
   //--- data
   ...
   static bool       IsAnyListShown(void) { return(s_list_shown); }
...
protected:
  ...
  bool CComboBox::s_list_shown = false; // Initialize static member
//+------------------------------------------------------------------+
...
//| Create a control                                                 |
//+------------------------------------------------------------------+
bool CComboBox::Create(const long chart,const string name,const int subwin, int x1,const int y1,const int x2,const int y2)
  {
//--- check height
...
bool CComboBox::ListShow(void)
  {
   if(CComboBox::IsAnyListShown())
      ListHide();
   BringToTop();
   CComboBox::s_list_shown = true;
//--- show the list
   flage = true;
   return(m_list.Show());
  }
...
{
    // If the event is not a mouse click or mouse move, return
   if (id != CHARTEVENT_CLICK && id != CHARTEVENT_MOUSE_MOVE)
      return;

   // If the list is not visible, return
   if (!m_list.IsVisible())
      return;

   // Adjust the coordinates based on the chart window
   y -= (int)ChartGetInteger(m_chart_id, CHART_WINDOW_YDISTANCE, m_subwin);

   // Check if the mouse click is inside the combo box or the list
   if (!m_edit.Contains(x, y) && !m_list.Contains(x, y))
   {
      // If the click is outside, hide the list
      ListHide();  // Use ListHide() to clear the static flag
      m_drop.Pressed(false);
   }
}