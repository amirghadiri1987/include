...
//| Resources                                                        |
//+------------------------------------------------------------------+
#resource "res\\Close_Panel.bmp"
#resource "res\\Up_Panel.bmp"
#resource "res\\Down_Panel.bmp"
#resource "res\\Sun_Panel.bmp"
#resource "res\\Moon_Panel.bmp"
#resource "res\\Close_PanelRED.bmp"
//+------------------------------------------------------------------+
//| Class CDialog                                                    |
...
protected:
   ...
   bool              m_moon;                // "create in moonsuned state" flag
   bool              m_sun;
   bool              is_hover_close;
   //--- additional areas
...
CDialog::CDialog(void) : m_panel_flag(false),
  ...
  {
   m_Themes.ApplyTheme((int)Sets.Themes);
  }
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
...
//--- save
   ...
   FileWriteInteger(file_handle,m_sun);
//--- result
...
//--- create
   ...
   if(!m_white_border.ColorBackground(CONTROLS_DIALOG_COLOR_BG))
      return(false);
   if(!m_white_border.ColorBorder(m_Themes.CONTROL_BORDER_BACKGROUND))
      return(false);
...
//--- create
   if(!m_background.Create(m_chart_id,m_name+"Back",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_background.ColorBackground(m_Themes.CONTROL_BACK_BACGROUND))
      return(false);
...
//--- coordinates
   int x1=off;
   int y1=off;
   int x2=Width()-off - 3*(CONTROLS_BUTTON_SIZE) - 3*(CUSTOM_SPACING);
   int y2=y1+CONTROLS_DIALOG_CAPTION_HEIGHT;
//--- create
   if(!m_caption.Create(m_chart_id,m_name+"Caption",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_caption.Color(m_Themes.CONTROL_CAPTION_TEXT))
      return(false);
   if(!m_caption.ColorBackground(m_Themes.CONTROL_CAPTION_BACKGROUND)) 
      return(false);
   if(!m_caption.ColorBorder(m_Themes.CONTROL_CAPTION_BORDERP))
      return(false);
...
//--- create
   if(!m_button_close.Create(m_chart_id,m_name+"Close",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_button_close.BmpNames("::res\\Close_Panel.bmp","::res\\Close_PanelRED.bmp"))
      return(false);
   // m_button_close.Color(clrRed);
...
//--- create
   if(!m_client_area.Create(m_chart_id,m_name+"Client",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_client_area.ColorBackground(m_Themes.CONTROL_CLINET_BACKGROUND))
      return(false);
   if(!m_client_area.ColorBorder(m_Themes.CONTROL_CLINET))
      return(false);
...
void CDialog::HandleCloseButtonHover(int mouse_x, int mouse_y)
{
   bool hover_close = m_button_close.OnMouseEvent(mouse_x, mouse_y, 0);

   if (hover_close && !is_hover_close)
   {
      m_button_close.BmpOffName("::res\\Close_PanelRED.bmp"); // Set to hover icon
      is_hover_close = true;
   }
   else if (!hover_close && is_hover_close)
   {
      m_button_close.BmpOffName("::res\\Close_Panel.bmp"); // Reset to default icon
...
public:
   ...
   virtual bool      Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2,const int x_minmiz);
   virtual void      Destroy(const int reason=REASON_PROGRAM);
   //--- chart event handler
   virtual bool      OnEvent(const int id,const long &lparam,const double &dparam,const string &sparam);
   virtual void      HandleChartChange(const double &dparam, const string &sparam);
   bool              IsPressedSunMode(void)             const {return m_button_moonsun.Pressed();}
   //--- dialog run
   bool              Run(void);
   //--- chart events processing
   void              ChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam);
   void              ChartEvent2(const int id,const long &lparam,const double &dparam,const string &sparam);
   //--- set up
...
private:
   ...
   bool              CreateExpert(const int x1,const int y1,const int x2,const int y2, const int x_minmiz);
   bool              CreateIndicator(const int x1,const int y1,const int x2,const int y2,const int x_minmiz);

protected:
   //--- create dependent controls
   virtual bool      CreateButtonMinMax(void);
   virtual bool      CreateButtonMoonSun(void);
   //--- handlers of the dependent controls events
   virtual void      OnClickButtonClose(void);
   virtual void      OnClickButtonMinMax(void);
   virtual void      OnClickButtonMoonsun(void);
   ...
   virtual void      Moon(void);
   virtual void      Sun(void);
...
ON_EVENT(ON_CLICK,m_button_moonsun,OnClickButtonMoonsun)
...
//+------------------------------------------------------------------+
bool CAppDialog::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2,const int x_minmiz)
  {
...
//---
   switch(m_program_type)
     {
      case PROGRAM_EXPERT:
         if(!CreateExpert(x1,y1,x2,y2,x_minmiz))
         return(false);
         break;
      case PROGRAM_INDICATOR:
         if(!CreateIndicator(x1,y1,x2,y2,x_minmiz))
         return(false);
         break;
      default:
...
//+------------------------------------------------------------------+
bool CAppDialog::CreateExpert(const int x1,const int y1,const int x2,const int y2,const int x_minmiz)
  {
...
//--- geometry for the minimized state
   m_min_rect.SetBound(CONTROLS_DIALOG_MINIMIZE_LEFT,
                       ...
                       CONTROLS_DIALOG_MINIMIZE_LEFT+x_minmiz,
...
//+------------------------------------------------------------------+
bool CAppDialog::CreateIndicator(const int x1,const int y1,const int x2,const int y2,const int x_minmiz)
  {
...
//--- dialog of such an indicator should behave as an Expert Advisor dialog
   if(m_subwin==0)
      return(CreateExpert(x1,y1,x2,y2,x_minmiz));
...
bool CAppDialog::CreateButtonMinMax(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
   int x1=Width()-off-2*(CONTROLS_BUTTON_SIZE+CONTROLS_DIALOG_BUTTON_OFF)-CUSTOM_SPACING;
   int y1=off+CONTROLS_DIALOG_BUTTON_OFF;
   int x2=x1+CONTROLS_BUTTON_SIZE;
   int y2=y1+CONTROLS_BUTTON_SIZE;
//--- create
   if(!m_button_minmax.Create(m_chart_id,m_name+"MinMax",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_button_minmax.BmpNames("::res\\Up_Panel.bmp","::res\\Down_Panel.bmp"))
      return(false);
   if(!CWndContainer::Add(m_button_minmax))
      return(false);
   m_button_minmax.Locking(true);
   m_button_minmax.Alignment(WND_ALIGN_RIGHT,0,0,off+(CONTROLS_BUTTON_SIZE+2*CONTROLS_DIALOG_BUTTON_OFF)+CUSTOM_SPACING,0);
//--- change caption
   CaptionAlignment(WND_ALIGN_WIDTH,off,0,off+2*(CONTROLS_BUTTON_SIZE+CONTROLS_DIALOG_BUTTON_OFF)+CUSTOM_SPACING,0);
//--- succeed
   return(true);
  }
//+------------------------------------------------------------------+
//| Create the "Moon/Sun" button                                     |
//+------------------------------------------------------------------+
bool CAppDialog::CreateButtonMoonSun(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
//--- coordinates
   int x1=Width()-off-3*(CONTROLS_BUTTON_SIZE+CONTROLS_DIALOG_BUTTON_OFF)-2*CUSTOM_SPACING;
   int y1=off+CONTROLS_DIALOG_BUTTON_OFF;
   int x2=x1+CONTROLS_BUTTON_SIZE;
   int y2=y1+CONTROLS_BUTTON_SIZE;
//--- create
   if(!m_button_moonsun.Create(m_chart_id,m_name+"MoonSun",m_subwin,x1,y1,x2,y2))
      return(false);
   if(!m_button_moonsun.BmpNames("::res\\Moon_Panel.bmp","::res\\Sun_Panel.bmp"))
      return(false);
   if(!CWndContainer::Add(m_button_moonsun))
      return(false);
   m_button_moonsun.Locking(true);
   m_button_moonsun.Alignment(WND_ALIGN_RIGHT,0,0,off+2*(CONTROLS_BUTTON_SIZE+2*CONTROLS_DIALOG_BUTTON_OFF+CUSTOM_SPACING)-CONTROLS_DIALOG_BUTTON_OFF,0);
//--- change caption
   CaptionAlignment(WND_ALIGN_WIDTH,off,0,off+3*(CONTROLS_BUTTON_SIZE+2*CONTROLS_DIALOG_BUTTON_OFF)+2*CUSTOM_SPACING,0);
//--- succeed
   return(true);
  }
//+------------------------------------------------------------------+
...
void CAppDialog::ChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   static bool is_hover_minmax = false;
   static bool is_hover_moonsun = false;
   ...
   switch (id)
   {
      ...
      case CHARTEVENT_MOUSE_MOVE:
      {
         ...
         HandleCloseButtonHover(mouse_x, mouse_y);
         // Handle hover effect for m_button_minmax
         bool hover_minmax = m_button_minmax.OnMouseEvent(mouse_x, mouse_y, (int)StringToInteger(sparam));
         bool hover_moonsun = m_button_moonsun.OnMouseEvent(mouse_x, mouse_y, (int)StringToInteger(sparam));

         if (m_minimized)
         {
            // Minimized state icons for minmax button
            if (hover_minmax && !is_hover_minmax)
            {
               m_button_minmax.BmpOnName("::res\\Up_Panel.bmp"); // Minimized Hover icon Close_PanelRED
               is_hover_minmax = true;
            }
            else if (!hover_minmax && is_hover_minmax)
            {
               m_button_minmax.BmpOnName("::res\\Down_Panel.bmp"); // Default icon
               is_hover_minmax = false;
            }
         }
         else // Not minimized
         {
            // Normal state icons for minmax button
            if (hover_minmax && !is_hover_minmax)
            {
               m_button_minmax.BmpOffName("::res\\Down_Panel.bmp"); // Normal Hover icon
               is_hover_minmax = true;
            }
            else if (!hover_minmax && is_hover_minmax)
            {
               m_button_minmax.BmpOffName("::res\\Up_Panel.bmp"); // Default icon
               is_hover_minmax = false;
            }

            // Sun/Moon mode icons for moonsun button
            if (m_sun)
            {
               if (hover_moonsun && !is_hover_moonsun)
               {
                  m_button_moonsun.BmpOnName("::res\\Moon_Panel.bmp"); // Hover icon for Sun mode
                  is_hover_moonsun = true;
               }
               else if (!hover_moonsun && is_hover_moonsun)
               {
                  m_button_moonsun.BmpOnName("::res\\Sun_Panel.bmp"); // Default icon for Sun mode
                  is_hover_moonsun = false;
               }
            }
            else // Moon mode
            {
               if (hover_moonsun && !is_hover_moonsun)
               {
                  m_button_moonsun.BmpOffName("::res\\Sun_Panel.bmp"); // Hover icon for Moon mode
                  is_hover_moonsun = true;
               }
               else if (!hover_moonsun && is_hover_moonsun)
               {
                  m_button_moonsun.BmpOffName("::res\\Moon_Panel.bmp"); // Default icon for Moon mode
                  is_hover_moonsun = false;
               }
            }
         }
...
void CAppDialog::OnClickButtonMinMax(void)
  {
   ...
   {
      Maximize();
      m_button_moonsun.Show();
   }
//--- get subwindow offset
   SubwinOff();
  }
//+------------------------------------------------------------------+
//| Handler of click on the "Moon/Sun" button                        |
//+------------------------------------------------------------------+
void CAppDialog::OnClickButtonMoonsun(void)
  {
   if(m_button_moonsun.Pressed()) { Sun();}
   else {Moon();}
...
void CAppDialog::Minimize(void)
  {
   int off=(m_panel_flag) ? 0:2*CONTROLS_BORDER_WIDTH;
   CaptionAlignment(WND_ALIGN_WIDTH,off,0,off+2*(CONTROLS_BUTTON_SIZE+2*CONTROLS_DIALOG_BUTTON_OFF)+CUSTOM_SPACING,0);
//--- set flag
   m_minimized=true;
   m_button_moonsun.Hide();
//--- resize
...
bool CAppDialog::Load(const int file_handle)
  {
   if(CDialog::Load(file_handle))
     {      
      ...
      if(m_sun)
         {
            m_button_moonsun.Pressed(true);
            Sun();
         }
      else
         {
            m_button_moonsun.Pressed(false);
            Moon();
         }
   ...
   if(CDialog::Save(file_handle))
     {
      ...
      FileWriteInteger(file_handle, m_sun);   // Save sun state
      return(true);
     }
//--- failure
   return(false);
